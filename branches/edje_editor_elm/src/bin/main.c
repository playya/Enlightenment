...
save_edje(const char *file)
{
  ...
  int save_success;
     ...
     {
        dialog_alert_show("<b>Warning</b><br>"
                 ...
                 "Are you sure you want to save the file? (<b>TODO:</b> Port Yes/No Dialog!");
     ...
     save_success = edje_edit_save(ui.edje_o);
     ...
     if (save_success)
     {
        if (file)
        {
           ...
           {
              // change open file name
              if (cur.open_file_name) eina_stringshare_del(cur.open_file_name);
              cur.open_file_name = eina_stringshare_add(file);
