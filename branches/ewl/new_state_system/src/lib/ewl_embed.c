...
ewl_embed_active_set(Ewl_Embed *embed, unsigned int act)
{
        ...
        if (e && e->last.clicked)
        {
                ...
                ewl_widget_state_remove2(e->last.clicked, EWL_STATE_FOCUSED);
                ewl_widget_state_remove2(e->last.clicked, EWL_STATE_MOUSE_DOWN);
                          ...
                          unsigned int mods)
{
        ...
        while (temp) {
                if (!DISABLED(temp)) {
                        ewl_widget_state_add2(temp, EWL_STATE_MOUSE_DOWN);
                        ...
                        int x, int y, unsigned int mods)
{
        ...
        while (temp) {
                if (!DISABLED(temp)) {
                        ewl_widget_state_remove2(temp, EWL_STATE_MOUSE_DOWN);
                        ewl_callback_call_with_event_data(temp,
...
ewl_embed_mouse_move_feed(Ewl_Embed *embed, int x, int y, unsigned int mods)
{
        ...
        if (!check || !ewl_widget_state_has2(check, EWL_STATE_MOUSE_DOWN)) {
        ...
        while (check && (widget != check)
                ...
                ewl_widget_state_remove2(check, EWL_STATE_MOUSE_IN);
        ...
        while (check) {
                if (!DISABLED(check)) {
                         ...
                         */
                        if (!(ewl_widget_state_has2(check, EWL_STATE_MOUSE_IN)))
                        {
        ...
        if (check && ewl_widget_state_has2(check, EWL_STATE_DND))
                ewl_callback_call_with_event_data(check,
                                                  EWL_CALLBACK_MOUSE_MOVE, &ev);
...
ewl_embed_mouse_out_feed(Ewl_Embed *embed, int x, int y, unsigned int mods)
{
        ...
        while (embed->last.mouse_in) {
                ewl_widget_state_remove2(embed->last.mouse_in,
                                        EWL_STATE_MOUSE_IN);
        ...
        if ((embed->last.drag_widget) && (ewl_widget_state_has2
...
ewl_embed_focused_widget_set(Ewl_Embed *embed, Ewl_Widget *w)
{
        ...
        {
                ewl_widget_state_remove2(embed->last.focused, EWL_STATE_FOCUSED);
        ...
        if (embed->last.focused)
        {
                ewl_widget_state_add2(embed->last.focused, EWL_STATE_FOCUSED);
                                        ...
                                        void *user_data __UNUSED__)
{
         ...
         * hence feed one */
        if (ewl_widget_state_has2(emb->last.focused, EWL_STATE_MOUSE_DOWN))
