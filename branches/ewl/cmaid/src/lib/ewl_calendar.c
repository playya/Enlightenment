                                                ...
                                                void *user_data)
{
        ...
        ewl_container_child_iterator_attach(EWL_CONTAINER(EWL_CALENDAR(user_data)->grid), &iter);
        while ((it = cmaid_iter_next(&iter)))
        {
                ewl_widget_color_set(EWL_WIDGET(it), 255, 255, 255, 255);
                ewl_calendar_highlight_today(now, EWL_LABEL(it), EWL_CALENDAR(user_data));
        }
        ewl_container_child_iterator_detach(EWL_CONTAINER(EWL_CALENDAR(user_data)->grid), &iter);
