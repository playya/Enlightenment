                                                ...
                                                void *user_data __UNUSED__)
{
        ...
        if (cmaid_container_empty_is(EWL_CONTAINER(w)->children))
                DRETURN(DLEVEL_STABLE);
...
ewl_box_configure_calc(Ewl_Box *b, Ewl_Object **spread, int *fill_size, int *align_size)
{
         ...
         */
        i = 0;
        cmaid_container_iter_attach(EWL_CONTAINER(b)->children, &iter);
        while ((child = cmaid_iter_next(&iter))) {
                int change;
                unsigned int policy;
        ...
        cmaid_container_iter_detach(EWL_CONTAINER(b)->children, &iter);
