...
static unsigned int ewl_callback_hash(const void *key, unsigned int *size);
static unsigned int ewl_callback_custom_hash(const void *key,
                                                unsigned int *size);
static int ewl_callback_compare(const void *key1, const void *key2);
static int ewl_callback_custom_compare(const void *key1, const void *key2);
...
static Cmaid_Cache *ewl_cb_cache = NULL;
static Cmaid_Cache *ewl_cb_custom_cache = NULL;

static const Cmaid_Value ewl_cb_value = {
...
static const Cmaid_Value ewl_cb_custom_value = {
        ewl_callback_custom_compare,
        ewl_callback_custom_hash,
        sizeof(Ewl_Callback_Custom),
        {NULL, NULL, NULL},
        {NULL, NULL}
};
...
ewl_callbacks_init(void)
{
        ...
        ewl_cb_cache = cmaid_hash_cache_new(&ewl_cb_value);
        ewl_cb_custom_cache = cmaid_hash_cache_new(&ewl_cb_custom_value);
...
ewl_callbacks_shutdown(void)
{
        ...
        IF_FREE_CONTAINER(ewl_cb_cache);
        IF_FREE_CONTAINER(ewl_cb_custom_cache);
...
ewl_callback_rm(Ewl_Widget *w, unsigned int t, unsigned int pos)
{
        ...
        {
                cmaid_cache_release(cache, w->callbacks[place].list);
        ...
        cmaid_cache_release(cache, w->callbacks[place].list[pos]);
                                ...
                                unsigned int pos, void *user_data)
{
        ...
        if (type < EWL_CALLBACK_MAX)
        {
                ...
                ret = cmaid_cache_instance(ewl_cb_cache, &cb);
        }
        else
        {
                ...
                ret = cmaid_cache_instance(ewl_cb_custom_cache, &cb);
        }

        ret = ewl_callback_insert(w, type, ret, pos);
...
/*
 * Hashes the value of a callback based on it's function and user data.
 */
static unsigned int
ewl_callback_hash(const void *key, unsigned int *size)
{
        Ewl_Callback *cb;
 ...
 */
static unsigned int
ewl_callback_custom_hash(const void *key, unsigned int *size)
{
        Ewl_Callback *cb;
        unsigned int hash;
        ...
        *size = sizeof(Ewl_Callback_Custom);
        hash = ecore_direct_hash(cb->func) ^ ecore_direct_hash(cb->user_data);
        hash = hash * 31 + EWL_CALLBACK_CUSTOM(cb)->event_id;
        DRETURN_INT(hash, DLEVEL_STABLE);
}
 ...
 */
static int
ewl_callback_custom_compare(const void *key1, const void *key2)
{
        Ewl_Callback *cb1;
        Ewl_Callback *cb2;
        ...
        if ((cb1->func == cb2->func) && (cb1->user_data == cb2->user_data)
                        && EWL_CALLBACK_CUSTOM(cb1)->event_id
                                == EWL_CALLBACK_CUSTOM(cb2)->event_id)
                DRETURN_INT(0, DLEVEL_STABLE);
