...
ewl_radiobutton_chain_set(Ewl_Radiobutton *rb, Ewl_Radiobutton *crb)
{
        ...
        if (!crb->chain)
        {
                crb->chain = cmaid_tree_set_new(NULL);
                cmaid_set_add(crb->chain, crb);
        }
        
        cmaid_set_add(crb->chain, rb);
        rb->chain = crb->chain;
...
ewl_radiobutton_chain_selected_get(Ewl_Radiobutton *rb)
{
         ...
         * treat it special */
        if (!rb->chain || cmaid_container_empty_is(rb->chain))
        {
        ...
        cmaid_container_iter_attach(rb->chain, &iter);
        while ((c = cmaid_iter_next(&iter)))
        {
                if (ewl_togglebutton_checked_get(EWL_TOGGLEBUTTON(c)))
                        break;
        }
        cmaid_container_iter_detach(rb->chain, &iter);
                                        ...
                                        void *user_data __UNUSED__)
{
        ...
        cmaid_set_remove(rb->chain, w);

        if (cmaid_container_empty_is(rb->chain))
                IF_FREE_CONTAINER(rb->chain);
