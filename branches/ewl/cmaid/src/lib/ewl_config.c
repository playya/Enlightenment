...
static int ewl_config_file_load(Ewl_Config *cfg, unsigned int is_system,
                                                        const char *file);
static void ewl_config_parse(Ewl_Config *cfg, Cmaid_Map *hash, char *data);

static Cmaid_Map *ewl_config_create_hash(void);
...
static Cmaid_Map *ewl_config_set_hash_get(Ewl_Config *cfg,
...
static int ewl_config_save(Ewl_Config *cfg, Cmaid_Map *hash, const char *file);
...
ewl_config_destroy(Ewl_Config *cfg)
{
        ...
        IF_FREE_CONTAINER(cfg->data.user);
        IF_FREE_CONTAINER(cfg->data.system);
        IF_FREE_CONTAINER(cfg->data.instance);
                                                ...
                                                Ewl_Durability state)
{
        ...
        cmaid_map_set(ewl_config_set_hash_get(cfg, state), (void *)k,
                        (void *)v);
                                        ...
                                        Ewl_Durability state)
{
        ...
        cmaid_map_set(ewl_config_set_hash_get(cfg, state), (void *)k, buf);
                                        ...
                                        Ewl_Durability state)
{
        ...
        cmaid_map_set(ewl_config_set_hash_get(cfg, state),
                                        (void *)k, buf);
...
ewl_config_instance_key_remove(Ewl_Config *cfg, const char *k)
{
        ...
        if (cfg->data.instance)
                cmaid_map_remove(cfg->data.instance, k);
...
ewl_config_system_key_remove(Ewl_Config *cfg, const char *k)
{
        ...
        if (cfg->data.system)
                cmaid_map_remove(cfg->data.system, k);
...
ewl_config_user_key_remove(Ewl_Config *cfg, const char *k)
{
        ...
        if (cfg->data.user)
                cmaid_map_remove(cfg->data.user, k);
...
static void
ewl_config_keys_remove_do(Cmaid_Container *cont, const char *k)
{
        ...
        cmaid_container_iter_attach(cont, &iter);
        while ((str = cmaid_iter_next(&iter)))
        {
                if (!strncmp(k, str, len))
                        cmaid_iter_remove(&iter);
        }
        cmaid_container_iter_detach(cont, &iter);
 ...
 */
static void
ewl_config_key_list_add(Cmaid_Container *cont, Ecore_List *list, const char *starts_with)
{
        Cmaid_Iter iter __CMAID_CHECKED_ITER__;
        char *str, *str_old;
        size_t len;
...
{
        Cmaid_Iter iter __CMAID_CHECKED_ITER__, it_val __CMAID_CHECKED_ITER__;
        char *fname, *key;
        int ret;
        Cmaid_Map *map;
        ...
        map = cmaid_hash_map_new(&cmaid_value_const_string,
                        &cmaid_value_const_string);

        /* add all the system data */
        cmaid_container_iter_attach(cfg->data.system, &iter);
        cmaid_map_value_iter_attach(cfg->data.system, &it_val);
        while ((key = cmaid_iter_next(&iter)))
        {
                cmaid_iter_iter_goto(&it_val, &iter);
                cmaid_map_set(map, key, cmaid_iter_current(&it_val));
        }
        cmaid_container_iter_detach(cfg->data.system, &iter);
        cmaid_container_iter_detach(cfg->data.system, &it_val);

        /* set the user data over top */
        cmaid_container_iter_attach(cfg->data.user, &iter);
        cmaid_map_value_iter_attach(cfg->data.user, &it_val);
        while ((key = cmaid_iter_next(&iter)))
        {
                cmaid_iter_iter_goto(&it_val, &iter);
                cmaid_map_set(map, key, cmaid_iter_current(&it_val));
        }
        cmaid_container_iter_detach(cfg->data.user, &iter);
        cmaid_container_iter_detach(cfg->data.user, &it_val);

        fname = ewl_config_file_name_system_get(cfg);
        ret = ewl_config_save(cfg, map, fname);
        FREE(fname);

        IF_FREE_CONTAINER(map);
...
static int
ewl_config_save(Ewl_Config *cfg, Cmaid_Map *map, const char *file)
{
        Cmaid_Iter iter __CMAID_CHECKED_ITER__, it_val __CMAID_CHECKED_ITER__;
        ...
        cmaid_container_iter_attach(map, &it_val);
        while ((key = cmaid_iter_next(&iter)))
        {
                int len;
                ...
                len = snprintf(data, sizeof(data), "%s = %s\n", key,
                                        (char *)cmaid_iter_current(&it_val));
                write(fd, data, len);
        }
...
ewl_config_create_hash(void)
{
        ...
        hash = cmaid_hash_map_new(&cmaid_value_inline_string,
...
ewl_config_get(Ewl_Config *cfg, const char *key)
{
        ...
        if (cfg->data.instance
                        && (val = cmaid_map_get(cfg->data.instance, key)))
        {
        ...
        else if ((val = ewl_config_env_get(cfg, key)))
        {
                /* we got our value, just NOP */
                        ...
                        && (val = cmaid_map_get(cfg->data.user, key)))
        {
                /* we got our value, just NOP */
        }
        else if (cfg->data.system
                        && (val = cmaid_map_get(cfg->data.system, key)))
        {
                /* we got our value, just NOP */
        }
...
/* Look up map values in the environment and cache the results. */
static const char *
...
ewl_config_file_load(Ewl_Config *cfg, unsigned int is_system, const char *file)
{
        ...
        /* create the map to store the values */
        if (is_system)
        {
...
static void
ewl_config_parse(Ewl_Config *cfg, Cmaid_Map *hash, char *data)
{
        char *start;
