...
extern Cmaid_List *ewl_window_list;
 ...
 */
static Cmaid_List *shutdown_queue = NULL;

/*
 * Queues for scheduling various actions.
 */
static Cmaid_List *obscure_list = NULL;
static Cmaid_List *reveal_list = NULL;
static Cmaid_List *realize_list = NULL;
static Cmaid_List *destroy_list = NULL;
static Cmaid_List *child_add_list= NULL;
...
/*
 * List of iterators that are waiting to be reused
 */
static Cmaid_Iter *iterator_cache = NULL;
...
ewl_init(int *argc, char **argv)
{
        ...
        shutdown_queue = cmaid_vector_list_new(NULL);
        if (!shutdown_queue) {
        ...
        cmaid_list_append(shutdown_queue, ecore_shutdown);
        ...
        cmaid_list_append(shutdown_queue, efreet_shutdown);

        if (!efreet_mime_init()) {
                fprintf(stderr, "Could not initialize Efreet_Mime.\n");
                goto FAILED;
        }
        cmaid_list_append(shutdown_queue, efreet_mime_shutdown);
#endif
        ...
        cmaid_list_append(shutdown_queue, ecore_string_shutdown);

        reveal_list = cmaid_vector_list_new(NULL);
        obscure_list = cmaid_vector_list_new(NULL);
        configure_active = ecore_list_new();
        configure_available = ecore_list_new();
        realize_list = cmaid_linked_list_new(NULL);
        destroy_list = cmaid_vector_list_new(NULL);
        free_evas_list = ecore_list_new();
        free_evas_object_list = ecore_list_new();
        child_add_list = cmaid_vector_list_new(NULL);
        ewl_embed_list = ecore_list_new();
        ewl_window_list = cmaid_linked_list_new(NULL);
        ...
        cmaid_list_append(shutdown_queue, ewl_system_directories_shutdown);
        ...
        cmaid_list_append(shutdown_queue, ewl_config_shutdown);

        if (!ewl_engines_init()) {
                fprintf(stderr, "Could not intialize Ewl Engines.\n");
                goto FAILED;
        }
        cmaid_list_append(shutdown_queue, ewl_engines_shutdown);
        ...
        cmaid_list_append(shutdown_queue, ewl_callbacks_shutdown);
...
ewl_shutdown(void)
{
        ...
        while (iterator_cache)
        {
                Cmaid_Iter *tmp;

                tmp = iterator_cache;
                iterator_cache = iterator_cache->next;
                FREE(tmp);
        }
         ...
         */
        IF_FREE_CONTAINER(ewl_window_list);
        IF_FREE_CONTAINER(reveal_list);
        IF_FREE_CONTAINER(obscure_list);
        IF_FREE_LIST(configure_active);
        IF_FREE_LIST(configure_available);
        IF_FREE_CONTAINER(realize_list);
        IF_FREE_CONTAINER(destroy_list);
        IF_FREE_LIST(free_evas_list);
        IF_FREE_LIST(free_evas_object_list);
        IF_FREE_CONTAINER(child_add_list);

        /* shutdown all the subsystems */
        while ((shutdown = cmaid_list_last(shutdown_queue)))
        {
                shutdown();
                cmaid_list_last_remove(shutdown_queue);
        }
        IF_FREE_CONTAINER(shutdown_queue);
...
ewl_idle_render(void *data __UNUSED__)
{
         ...
         */
        if (!cmaid_container_empty_is(destroy_list) ||
        ...
        if (!cmaid_container_empty_is(realize_list))
                ewl_realize_queue();
        ...
        while (!ecore_list_empty_is(configure_active)) {
                 ...
                 */
                while ((w = cmaid_list_last(obscure_list)))
                {
                        cmaid_list_last_remove(obscure_list);
                        /*
                 ...
                 */
                while ((w = cmaid_list_first(reveal_list)))
                {
                        cmaid_list_first_remove(reveal_list);
                        /*
...
ewl_configure_queue_widget_run(Ewl_Widget *w)
{
        ...
        if (!ewl_widget_onscreen_is(w)) {
                if (REVEALED(w))
                        cmaid_list_append(obscure_list, w);
        }
        else {
                if (!REVEALED(w))
                        cmaid_list_append(reveal_list, w);
...
ewl_realize_request(Ewl_Widget *w)
{
        ...
        cmaid_list_append(realize_list, w);
...
ewl_realize_cancel_request(Ewl_Widget *w)
{
        ...
        cmaid_container_iter_attach(realize_list, &iter);
        /* find the widget */
        while (w != cmaid_iter_next(&iter) && cmaid_iter_current(&iter))
                ;
        if (cmaid_iter_current(&iter) == w)
        {
                ewl_widget_queued_remove(w, EWL_FLAG_QUEUED_SCHEDULED_REVEAL);
                cmaid_iter_remove(&iter);
        }

        cmaid_container_iter_detach(realize_list, &iter);
...
ewl_realize_queue(void)
{
         ...
         */
        while ((w = cmaid_list_first(realize_list)))
        {
                cmaid_list_first_remove(realize_list);
                if (VISIBLE(w) && !REALIZED(w)) {
                        ...
                        ewl_widget_queued_remove(w,
                                                EWL_FLAG_QUEUED_PROCESS_REVEAL);
                        cmaid_list_append(child_add_list, w);
                }
        }
         ...
         * top.
         */
        while ((w = cmaid_list_last(child_add_list)))
        {
                cmaid_list_last_remove(child_add_list);
                /*
...
ewl_destroy_request(Ewl_Widget *w)
{
        ...
        /*
         * Must append to ensure children are freed before parents.
         */
        cmaid_list_append(destroy_list, w);
...
ewl_garbage_collect_idler(void *data __UNUSED__)
{
        ...
        while ((cleanup < EWL_GC_LIMIT) && (w = cmaid_list_last(destroy_list)))
        {
                cmaid_list_last_remove(destroy_list);
        ...
        if ((ecore_list_count(free_evas_object_list) == 0)
                        && (cmaid_container_empty_is(destroy_list)))
        {
        ...
        DRETURN_INT(cmaid_container_count(destroy_list), DLEVEL_STABLE);
}
...
ewl_iterator_alloc(void)
{
        ...
        if (iterator_cache)
        {
                ...
                ret = iterator_cache;
                iterator_cache = ret->next;
                DRETURN_PTR(ret, DLEVEL_STABLE);
        }

        DRETURN_PTR(NEW(Cmaid_Iter, 1), DLEVEL_STABLE);
}

void
ewl_iterator_free(Cmaid_Iter * iter)
{
        DENTER_FUNCTION(DLEVEL_STABLE);

        iter->next = iterator_cache;
        iterator_cache = iter;
