...
ewl_statusbar_init(Ewl_Statusbar *sb)
{
        ...
        sb->stack = cmaid_vector_list_new(NULL);
...
ewl_statusbar_push(Ewl_Statusbar *sb, const char *txt)
{
        ...
        cmaid_list_append(sb->stack, sb->current);
...
ewl_statusbar_pop(Ewl_Statusbar *sb)
{
        ...
        /* remove the last */
        current = cmaid_list_last(sb->stack);
        if (current)
        {
                cmaid_list_last_remove(sb->stack);
