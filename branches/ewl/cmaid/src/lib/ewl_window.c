...
Cmaid_List *ewl_window_list = NULL;
...
ewl_window_init(Ewl_Window *w)
{
        ...
        cmaid_list_append(ewl_window_list, w);
...
ewl_window_window_find(void *window)
{
        ...
        cmaid_container_iter_attach(ewl_window_list, &iter);
        while ((retwin = cmaid_iter_next(&iter)))
                if (retwin->window == window)
                        break;
        cmaid_container_iter_detach(ewl_window_list, &iter);
                                        ...
                                        void *user_data __UNUSED__)
{
        ...
        cmaid_container_iter_attach(ewl_window_list, &iter);
        while ((win = cmaid_iter_next(&iter)))
        {
                if (win == EWL_WINDOW(w))
                {
