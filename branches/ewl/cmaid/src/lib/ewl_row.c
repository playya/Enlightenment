...
ewl_row_column_get(Ewl_Row *row, short n)
{
        ...
        cmaid_container_iter_attach(EWL_CONTAINER(row)->children, &iter);
        found = cmaid_iter_index_goto(&iter, n + 1);
        cmaid_container_iter_detach(EWL_CONTAINER(row)->children, &iter);
                                        ...
                                        void *user_data __UNUSED__)
{
        ...
        Ewl_Object *child;
        Ewl_Object *align;
        Cmaid_Iter c_it __CMAID_CHECKED_ITER__;
        int x;
        int remains, nodes;
        ...
        nodes = 0;
        ewl_container_child_iterator_attach(c, &c_it);
        while ((child = cmaid_iter_next(&c_it))) {
                if (VISIBLE(child))
                        nodes++;
        }
        
        cmaid_iter_start(&c_it);
        ...
        if (row->header) {
                int width;
                Cmaid_Iter h_it __CMAID_CHECKED_ITER__;

                ewl_container_child_iterator_attach(row->header, &h_it);

                /*
                 * Get the first child of the header.
                 */
                align = cmaid_iter_next(&h_it);
                if (align)
                ...
                ewl_container_child_iterator_detach(row->header, &h_it);
        }
        /*
         ...
         */
                ...
                if (remains) {
                        Cmaid_Iter iter __CMAID_CHECKED_ITER__;
                        tx = x;
                        nodes = cmaid_container_count(c->children);
        ...
        ewl_container_child_iterator_detach(c, &c_it);
