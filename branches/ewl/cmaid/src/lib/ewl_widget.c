...
typedef struct Ewl_Widget_Data_Pair Ewl_Widget_Data_Pair;

struct Ewl_Widget_Data_Pair
{
        Ewl_Widget *w;
        void *data;
};

static Cmaid_Map *ewl_widget_name_table = NULL;
static Cmaid_Map *ewl_widget_data_table = NULL;

static int ewl_widget_data_compare(const void *v1, const void *v2);
static unsigned int ewl_widget_data_hash(const void *v, unsigned int *size);

static const Cmaid_Value ewl_widget_data_value =
{
        ewl_widget_data_compare,
        ewl_widget_data_hash,
        sizeof(Ewl_Pair),
...
ewl_widget_name_set(Ewl_Widget *w, const char *name)
{
        ...
        {
                ewl_widget_name_table = cmaid_hash_map_new(
        ...
        if (name)
        {
                ...
                t = cmaid_map_set(ewl_widget_name_table, (void *)name, w);
                ewl_attach_name_set(w, t);
        }
        else if ((old_name = ewl_attach_name_get(w)))
        {
                cmaid_map_remove(ewl_widget_name_table, old_name);
                ewl_attach_name_set(w, NULL);
        }
...
ewl_widget_name_find(const char *name)
{
        ...
        if (ewl_widget_name_table)
                match = EWL_WIDGET(cmaid_map_get(ewl_widget_name_table, name));
...
ewl_widget_data_set(Ewl_Widget *w, void *k, void *v)
{
        ...
        if (!ewl_widget_data_table)
                ewl_widget_data_table = cmaid_hash_map_new(
                                                &ewl_widget_data_value,
                                                &cmaid_value_pointer);

        cmaid_map_set(ewl_widget_data_table, &pair, v);

        DLEAVE_FUNCTION(DLEVEL_STABLE);
}

static int
ewl_widget_data_compare(const void *v1, const void *v2)
{
        int comp;
        const Ewl_Widget_Data_Pair *val1, *val2;
        ...
        comp = cmaid_value_pointer_compare(val1->w, val2->w);
        if (comp == 0)
                DRETURN_INT(cmaid_value_pointer_compare(val1->data, val2->data),
                                DLEVEL_STABLE);

        DRETURN_INT(comp, DLEVEL_STABLE);
}

static unsigned int
ewl_widget_data_hash(const void *v, unsigned int *size)
{
        const Ewl_Widget_Data_Pair *pair;
        unsigned int ret;
        unsigned int dummy;
        ...
        ret = cmaid_value_pointer_hash(pair->w, &dummy) * 33
                ^ cmaid_value_pointer_hash(pair->data, &dummy);
...
ewl_widget_data_del(Ewl_Widget *w, void *k)
{
        ...
        v = cmaid_map_get(ewl_widget_data_table, &pair);

        if (v)
                cmaid_map_remove(ewl_widget_data_table, &pair);

        DRETURN_PTR(v, DLEVEL_STABLE);
}
...
ewl_widget_data_get(Ewl_Widget *w, void *k)
{
        ...
        data = cmaid_map_get(ewl_widget_data_table, &p);
...
ewl_widget_name_table_shutdown(void)
{
        ...
        IF_FREE_CONTAINER(ewl_widget_name_table);
