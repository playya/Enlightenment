 ...
 */
static void  ewl_container_iter_start(Cmaid_Iter *it);
static void *ewl_container_iter_iter_goto(Cmaid_Iter *it,
                                                const Cmaid_Iter *to_it);
static void *ewl_container_iter_index_goto(Cmaid_Iter *it, int index);
static void *ewl_container_iter_current(Cmaid_Iter *it);
static void *ewl_container_iter_next(Cmaid_Iter *it);
static void *ewl_container_iter_previous(Cmaid_Iter *it);
static void  ewl_container_iter_remove(Cmaid_Iter *it);
/*
 * iterator interface
 */
const Cmaid_Iter_Interface ewl_container_iter_if =
{
        ewl_container_iter_start,
        ewl_container_iter_iter_goto,
        ewl_container_iter_index_goto,
        ewl_container_iter_current,
        ewl_container_iter_next,
        ewl_container_iter_previous,
        ewl_container_iter_remove
};
...
ewl_container_init(Ewl_Container *c)
{
         ...
         */
        c->children = cmaid_linked_list_new(NULL);
...
ewl_container_child_append(Ewl_Container *pc, Ewl_Widget *child)
{
        ...
        if (!ewl_widget_internal_is(child))
                pc->external_children++;
        cmaid_list_append(pc->children, child);
...
ewl_container_child_prepend(Ewl_Container *pc, Ewl_Widget *child)
{
        ...
        if (!ewl_widget_internal_is(child))
                pc->external_children++;
        cmaid_list_prepend(pc->children, child);
        ewl_container_child_add_call(pc, child);
                                                ...
                                                int index, int skip_internal)
{
        ...
        cmaid_container_iter_attach(pc->children, &iter);
        while ((cur = cmaid_iter_next(&iter)))
        {
        ...
        ewl_widget_parent_set(child, EWL_WIDGET(pc));
        cmaid_list_insert_before(pc->children, &iter, child);
        if (!ewl_widget_internal_is(child))
                pc->external_children++;
        ewl_container_child_add_call(pc, child);
        cmaid_container_iter_detach(pc->children, &iter);
...
ewl_container_child_remove(Ewl_Container *pc, Ewl_Widget *child)
{
         ...
         */
        cmaid_container_iter_attach(pc->children, &iter);
        while ((temp = cmaid_iter_next(&iter)) && temp != child)
                if (!ewl_widget_internal_is(temp))
                        idx_ext++;
        ...
        idx = cmaid_iter_index_get(&iter);

        /*
         * before we remove the child we update the iterators in the list
         */
        if (!ewl_widget_internal_is(child))
        {
                Cmaid_Iter *it;
                for (it = pc->iter_list; it; it = it->next)
                {
                        if (it->index > idx_ext)
                                it->index--;
                        else if (it->index == idx_ext)
                                ewl_container_iter_previous(it);
                }
        }
...
ewl_container_child_count_get(Ewl_Container *c)
{
        ...
        while (c->redirect) c = c->redirect;

        DRETURN_INT(c->external_children, DLEVEL_STABLE);
}
 ...
 * @brief Returns the number of child widgets in the container
 *
 * @note This is not function do not only return the count of the internal
 *       widgets, but the total count.
 */
int
ewl_container_child_count_internal_get(Ewl_Container *c)
{
        ...
        DRETURN_INT(cmaid_container_count(c->children), DLEVEL_STABLE);
}
                                                ...
                                                unsigned int skip)
{
        ...
        cmaid_container_iter_attach(container->children, &iter);
        while ((child = cmaid_iter_next(&iter))) {
                if (skip && ewl_widget_internal_is(child)) continue;
                if (count == index) break;
                count ++;
        }
        cmaid_container_iter_detach(container->children, &iter);
...
ewl_container_child_at_get(Ewl_Container *widget, int x, int y)
{
        ...
        if (!widget->children || cmaid_container_empty_is(widget->children))
                DRETURN_PTR(NULL, DLEVEL_STABLE);

        cmaid_container_iter_attach(widget->children, &iter);
        /*
        ...
        cmaid_container_iter_detach(widget->children, &iter);
...
ewl_container_reset(Ewl_Container *c)
{
         ...
         */
        cmaid_container_iter_attach(c->children, &iter);
        while ((w = cmaid_iter_next(&iter))) {
        ...
        cmaid_container_iter_detach(c->children, &iter);
 ...
 */
void
ewl_container_child_iterator_attach(Ewl_Container *c, Cmaid_Iter *iter)
{
        DENTER_FUNCTION(DLEVEL_STABLE);
        DCHECK_PARAM_PTR(c);
        DCHECK_PARAM_PTR(iter);
        DCHECK_TYPE(c, EWL_CONTAINER_TYPE);

        while (c->redirect)
                c = c->redirect;

        iter->current = ewl_iterator_alloc();
        if (!iter->current)
                DRETURN(DLEVEL_STABLE);

        cmaid_container_iter_attach(c->children, iter->current);
        iter->container = c;
        iter->index = -1;
        iter->interface = &ewl_container_iter_if;

        /* push the iter into the iter list */
        iter->next = c->iter_list;
        c->iter_list = iter;
 ...
 * @return Returns no value.
 * @brief Detach an iterator to the container
 */
void
ewl_container_child_iterator_detach(Ewl_Container *c, Cmaid_Iter *iter)
{
        Cmaid_Iter **last;
        ...
        if (c != iter->container)
        {
                DWARNING("the iterator is not attached to this  container\n");
                DRETURN(DLEVEL_STABLE);
        }
        ...
        *last = (*last)->next;
        ...
        cmaid_container_iter_detach(c->children, iter->current);
        ewl_iterator_free(iter->current);
        ZERO(iter, *iter, 1);
...
ewl_container_child_show_call(Ewl_Container *c, Ewl_Widget *w)
{
        ...
        c->visible_children++;
        if (c->visible_children > cmaid_container_count(c->children))
        {
                DWARNING("visible children count exceed total child count "
                                "(%d > %d)\n", c->visible_children,
                                cmaid_container_count(c->children));
...
ewl_container_destroy(Ewl_Container *c)
{
        ...
        if (c->children) {
                 ...
                 */
                while ((child = cmaid_list_first(c->children))) {
                 ...
                 */
                cmaid_container_destroy(c->children);
                c->children = NULL;
        }
                                                ...
                                                void *user_data __UNUSED__)
{
         ...
         */
        cmaid_container_iter_attach(EWL_CONTAINER(w)->children, &iter);
        while ((child = cmaid_iter_next(&iter))) {
                ewl_widget_reparent(child);
        }
        cmaid_container_iter_detach(EWL_CONTAINER(w)->children, &iter);
                                                ...
                                                void *user_data __UNUSED__)
{
        ...
        if (!c->children || cmaid_container_empty_is(c->children))
                DRETURN(DLEVEL_STABLE);
         ...
         */
        /* XXX we used ecore_dlist_index_goto() to iterator over the list,
         *  this smells like  O(N²), check this and fix it if possible */
        cmaid_container_iter_attach(c->children, &iter);
        while ((child = cmaid_iter_next(&iter))) {
...
ewl_container_children_show(Ewl_Container *c)
{
        ...
        ewl_container_child_iterator_attach(c, &iter);
        while ((w = cmaid_iter_next(&iter)))
        {
        ...
        ewl_container_child_iterator_detach(c, &iter);
 ...
 */
static void
ewl_container_iter_start(Cmaid_Iter *it)
{
        it->index = -1;
        cmaid_iter_start(it->current);
}

static void *
ewl_container_iter_iter_goto(Cmaid_Iter *it, const Cmaid_Iter *to_it)
{
        if (!to_it || it->container != to_it->container)
                return NULL;

        it->index = to_it->index;
        return cmaid_iter_iter_goto(it->current, to_it->current);
}

static void *
ewl_container_iter_index_goto(Cmaid_Iter *it, int index)
{
        Ewl_Container *c = it->container;
        int i;

        if (c->external_children == 0)
                return NULL;

        if (index >= c->external_children || index < 0)
                return NULL;

        if (it->index < 0)
        {
                if (index > c->external_children/2)
                        ewl_container_iter_previous(it);
                else
                        ewl_container_iter_next(it);
        }
        ...
        return cmaid_iter_current(it->current);
}

static void *
ewl_container_iter_current(Cmaid_Iter *it)
{
        return cmaid_iter_current(it->current);
}

static void *
ewl_container_iter_next(Cmaid_Iter *it)
{
        Ewl_Widget *child;

        while ((child = cmaid_iter_next(it->current))
                        && ewl_widget_internal_is(child))
                ;
        if (child)
                it->index++;
        else
...
static void *
ewl_container_iter_previous(Cmaid_Iter *it)
{
        Ewl_Widget *child;

        if (!cmaid_iter_current(it->current))
                it->index = EWL_CONTAINER(it->container)->external_children;

        it->index--;
        while ((child = cmaid_iter_previous(it->current))
                        && ewl_widget_internal_is(child))
                ;
        return child;
}
...
static void
ewl_container_iter_remove(Cmaid_Iter *it)
{
        Ewl_Widget *child;
        
        if ((child = cmaid_iter_current(it->current)))
                ewl_widget_destroy(child);
}
