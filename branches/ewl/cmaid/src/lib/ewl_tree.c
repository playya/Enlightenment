...
ewl_tree_headers_build(Ewl_Tree *tree, const Ewl_Model *model, void *mvc_data)
{
        ...
        ewl_container_child_iterator_attach(header, &iter);
        for (i = 0; i < tree->columns; i++)
        {
                Ewl_Widget *h = cmaid_iter_next(&iter);
        ...
        ewl_container_child_iterator_detach(header, &iter);
...
ewl_tree_node_expand(Ewl_Tree_Node *node)
{
        ...
        cmaid_container_iter_attach(EWL_CONTAINER(node)->children, &iter);
        while ((child = cmaid_iter_next(&iter)))
        {
                if ((child != node->handle) && (child != EWL_WIDGET(node->row))
                                && !UNMANAGED(child))
                        /* XXX we had stored the children in a tmp list
                         * because the children list, with cmaid this should
                         * not be a problem, but it sounds like O(N^2)
                         */
                        ewl_widget_show(child);
        }
        cmaid_container_iter_detach(EWL_CONTAINER(node)->children, &iter);
...
ewl_tree_cb_node_child_hide(Ewl_Container *c, Ewl_Widget *w)
{
        ...
        if (cmaid_container_count(c->children) < 3)
        {
...
ewl_tree_cb_node_child_add(Ewl_Container *c, Ewl_Widget *w __UNUSED__)
{
        ...
        if (cmaid_container_count(c->children) > 2)
        {
