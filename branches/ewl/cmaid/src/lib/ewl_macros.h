...
#undef IF_FREE_CONTAINER
/**
 * @def IF_FREE_CONTAINER(item)
 * If @a item is non-NULL, cmaid_container_destroy @a item and set it NULL.
 */
#define IF_FREE_CONTAINER(item) \
do { \
        if (item) { \
                cmaid_container_destroy(item); \
                item = NULL; \
        } \
} while (0)
