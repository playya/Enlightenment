...
const static Cmaid_Value ewl_theme_string_value = {
        cmaid_value_string_compare, cmaid_value_string_hash, 0,
        {NULL, NULL, NULL},
        {CMAID_VALUE_CLONE_CB(ecore_string_instance),
                CMAID_VALUE_FREE_CB(ecore_string_release)}
};
...
ewl_theme_init(void)
{
        ...
        if (!EWL_THEME_KEY_NOMATCH)
                EWL_THEME_KEY_NOMATCH = ecore_string_instance("__nomatch");
...
ewl_theme_shutdown(void)
{
        ...
        IF_FREE_CONTAINER(ewl_theme_def_data);

        FREE(ewl_theme_path);
        IF_RELEASE(EWL_THEME_KEY_NOMATCH);
...
ewl_theme_theme_set(const char *theme_name)
{
        ...
        ewl_theme_def_data = cmaid_hash_map_new(&ewl_theme_string_value,
                        &ewl_theme_string_value);
...
static void
ewl_theme_lookup_cache(Cmaid_Map *cache, const char *k, const char *v)
{
        DENTER_FUNCTION(DLEVEL_STABLE);
        DCHECK_PARAM_PTR(cache);
        DCHECK_PARAM_PTR(k);

        cmaid_map_set(cache, (void *)k, (void *)v);

        DLEAVE_FUNCTION(DLEVEL_STABLE);
}

static const char *
ewl_theme_lookup_key(Cmaid_Map *cache, const char *path, const char *k)
{
        const char *ret;
...
ewl_theme_data_str_set(Ewl_Widget *w, const char *k, const char *v)
{
        ...
        if (!w->theme || w->theme == ewl_theme_def_data)
                w->theme = cmaid_hash_map_new(&ewl_theme_string_value,
                                &ewl_theme_string_value);

        if (!v)
                v = EWL_THEME_KEY_NOMATCH;

        cmaid_map_set(w->theme, (void *)k, (void *)v);
