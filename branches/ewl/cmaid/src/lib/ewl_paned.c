...
ewl_paned_orientation_set(Ewl_Paned *p, Ewl_Orientation o)
{
        ...
        /* loop over all the children and change the orientation of all
          * of the grabbers */
        cmaid_container_iter_attach(EWL_CONTAINER(p)->children, &iter);
        ...
        cmaid_container_iter_detach(EWL_CONTAINER(p)->children, &iter);
...
ewl_paned_grapper_size_get(Ewl_Paned *p)
{
        ...
        while ((child = cmaid_iter_next(&iter)))
        {
                ...
                ret = layout->current_size(EWL_OBJECT(child));
                break;
        }
        cmaid_container_iter_detach(c->children, &iter);
...
ewl_paned_grabber_cb_mouse_move(Ewl_Widget *w, void *ev, void *data)
{
        ...
        cmaid_container_iter_attach(c->children, &iter);
        while ((child = cmaid_iter_next(&iter)) && child != w)
                ;

        /* move past the selected grabber */
        while ((child = cmaid_iter_previous(&iter)))
        {
