...
ewl_embed_init(Ewl_Embed *emb)
{
        ...
        emb->obj_cache = cmaid_hash_map_new(&cmaid_value_const_string, NULL);
...
ewl_embed_object_cache(Ewl_Embed *e, void *obj)
{
        ...
        if (e->obj_cache) {
                ...
                obj_list = cmaid_map_get(e->obj_cache, type);
                if (!obj_list) {
                        obj_list = ecore_list_new();
                        cmaid_map_set(e->obj_cache, (char *)type, obj_list);
                }
...
ewl_embed_cache_cleanup(Ewl_Embed *emb)
{
        ...
        cmaid_container_iter_detach(emb->obj_cache, &iter);

        IF_FREE_CONTAINER(emb->obj_cache);
