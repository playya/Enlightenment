...
static int test_iterator(char *buf, int len);
...
Ewl_Unit_Test container_unit_tests[] = {
		...
		{"Iterator", test_iterator, NULL, -1, 0},
                {NULL, NULL, NULL, -1, 0}
        };
...
test_child_append(char *buf, int len)
{
	...
	Cmaid_Iter iter;
        ...
        ewl_container_child_iterator_attach(EWL_CONTAINER(c), &iter);
        ...
        ewl_container_child_iterator_detach(EWL_CONTAINER(c), &iter);
...
static int test_redirect_set_get(char *buf, int len)
{
	...
	Cmaid_Iter iter;
        ...
        ewl_container_child_iterator_attach(EWL_CONTAINER(c2), &iter);
        if (cmaid_iter_next(&iter) != w)
        {
        ...
        ret = 1;
DONE2:
        ewl_container_child_iterator_detach(EWL_CONTAINER(c2), &iter);
DONE:
...
int
test_iterator(char *buf, int size)
{
	const int widget_count = 100;
	Ewl_Container *c;
	Ewl_Widget *w;
	Cmaid_Iter last __CMAID_CHECKED_ITER__;
	Cmaid_Iter first __CMAID_CHECKED_ITER__;
	int ext_children;
	int ret = 0;
	int i;

	c = EWL_CONTAINER(ewl_container_new());
	...
	for (i = 0; i < widget_count; i++)
	{
		...
		if (i % 2)
			ewl_widget_internal_set(w, TRUE);
                        ...
                        ext_children++;
		ewl_container_child_append(c, w);
	}
	ewl_container_child_iterator_attach(c, &last);
	cmaid_iter_previous(&last);
	ewl_container_child_iterator_attach(c, &first);

	/* remove all external children */
	while ((w = cmaid_iter_next(&first)))
	{
		if (ewl_widget_internal_is(w))
		{
			LOG_FAILURE(buf, size, "found a internal widget");
			goto DONE;
		}
		cmaid_iter_remove(&first);
		ext_children--;
		if (cmaid_iter_index_get(&last) != ext_children - 1)
		{
			LOG_FAILURE(buf, size, "index is %d but should be %d",
					cmaid_iter_index_get(&last),
					ext_children - 1);
			goto DONE;
		}
	}
	ret = 1;
DONE:
	ewl_container_child_iterator_detach(c, &first);
	ewl_container_child_iterator_detach(c, &last);
